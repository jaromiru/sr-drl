...
class AStar:
    ...
    def astar(self, start, goal, reversePath=False):
        ...
        while openSet:
            ...
            # print(f"{current.data} {goal} g={current.gscore}, h={current.fscore - current.gscore}, f={current.fscore}")
            # print("Current heap:", list(map(lambda x: (set(x.data), f"g={x.gscore}, h={x.fscore - x.gscore}, f={x.fscore}"), openSet)))
            # print()
            if self.is_goal_reached(current.data, goal):
                return self.reconstruct_path(current, reversePath), current.gscore
